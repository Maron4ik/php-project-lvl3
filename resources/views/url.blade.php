<html debug="true"><div id="FirebugChannel" style="display: none;"></div><script src="chrome-extension://ehemiojjcpldeipjhjkepfdaohajpbdo/csp-test.js" id="FirebugLiteCSPTest" firebugignore="true" extension="Chrome" extension-id="ehemiojjcpldeipjhjkepfdaohajpbdo" csp-mode="true"></script><head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- CSRF Token -->
    <meta name="csrf-token" content="BU8dfaSnQnMvrRFNaEEUe1lNTzjB9Sdcibv6kv0F">

    <title>Анализатор страниц</title>

    <!-- Scripts -->
    <script src="https://php-l3-page-analyzer.herokuapp.com/js/app.js" defer=""></script>

    <!-- Fonts -->
    <link rel="dns-prefetch" href="//fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css?family=Nunito" rel="stylesheet">

    <!-- Styles -->
    <link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css">
    <link href="https://php-l3-page-analyzer.herokuapp.com/css/app.css" rel="stylesheet">
    <style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: sc-global-2423173963 */
        @font-face{font-family:'Open Sans';font-display:swap;font-weight:300;font-style:normal;src:local('Open Sans Light'), url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Light.ttf");} @font-face{font-family:'Open Sans';font-display:swap;font-weight:400;font-style:normal;src:local('Open Sans Regular'), url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Regular.ttf");} @font-face{font-family:'Open Sans';font-display:swap;font-weight:600;font-style:normal;src:local('Open Sans SemiBold'), url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-SemiBold.ttf");} @font-face{font-family:'Roboto Bold';font-weight:700;font-style:normal;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/Roboto-Bold.ttf");} @font-face{font-family:'Open Sans';font-style:italic;font-weight:300;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-LightItalic.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:italic;font-weight:400;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-RegularItalic.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:italic;font-weight:600;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-SemiBoldItalic.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:italic;font-weight:700;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-BoldItalic.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:italic;font-weight:800;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-ExtraBoldItalic.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:normal;font-weight:300;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Light.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:normal;font-weight:400;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Regular.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:normal;font-weight:600;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-SemiBold.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:normal;font-weight:700;font-display:swap;src:local('Open Sans Bold'), url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Bold.ttf");} @font-face{font-family:'Open Sans';font-style:normal;font-weight:700;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-Bold.ttf") format('truetype');} @font-face{font-family:'Open Sans';font-style:normal;font-weight:800;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/OpenSans-ExtraBold.ttf") format('truetype');} @font-face{font-family:'Roboto';font-style:normal;font-weight:400;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/Roboto-Regular.ttf") format('truetype');} @font-face{font-family:'Roboto';font-style:normal;font-weight:500;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/Roboto-Medium.ttf") format('truetype');} @font-face{font-family:'Roboto';font-style:normal;font-weight:700;font-display:swap;src:url("chrome-extension://pfjibkklgpfcfdlhijfglamdnkjnpdeg/assets/fonts/Roboto-Bold.ttf") format('truetype');} .apexcharts-tooltip{background:rgba(255,255,255,0.88) !important;mix-blend-mode:normal !important;box-shadow:0 2px 10px rgba(9,19,80,0.12316), 0 3px 2px rgba(128,128,128,0.058707), inset 0 0 3px rgba(255,255,255,0.5) !important;-webkit-backdrop-filter:blur(1px) !important;backdrop-filter:blur(1px) !important;border-radius:4px !important;} .rtooltip-wrapper{margin-left:10px;margin-right:10px;padding-top:10px;padding-bottom:8px;display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-direction:column;-ms-flex-direction:column;flex-direction:column;} #rootSkeleton{position:absolute;z-index:9999999;} .rtooltip-price{font-family:'Open Sans',sans-serif;font-style:normal;font-weight:bold;font-size:14px;line-height:12px;-webkit-letter-spacing:-0.3px;-moz-letter-spacing:-0.3px;-ms-letter-spacing:-0.3px;letter-spacing:-0.3px;color:#000000;} .rtooltip-date{padding-top:4px;font-family:'Open Sans',sans-serif;font-style:normal;font-weight:normal;font-size:12px;line-height:15px;-webkit-letter-spacing:-0.3px;-moz-letter-spacing:-0.3px;-ms-letter-spacing:-0.3px;letter-spacing:-0.3px;color:#424242;opacity:0.5;} .rtooltip-info{padding-top:5px;width:110px;height:34px;white-space:initial;font-family:'Open Sans',sans-serif;font-style:normal;font-weight:normal;font-size:11px;line-height:14px;-webkit-letter-spacing:-0.3px;-moz-letter-spacing:-0.3px;-ms-letter-spacing:-0.3px;letter-spacing:-0.3px;} .rtooltip-price-strong{font-weight:bold;} .apexcharts-tooltip.light{border:none !important;} .apexcharts-tooltip.active{opacity:1;-webkit-transition:0.5s ease all !important;transition:0.5s ease all !important;} .apexcharts-xcrosshairs.active,.apexcharts-ycrosshairs.active{opacity:1;-webkit-transition:0.5s ease all;transition:0.5s ease all;} .apexcharts-series-markers-wrap > g:not(:last-of-type) circle[r='4.6']{fill:white;stroke:#4da1ff;}
        /* sc-component-id: Font-sc-zlhigq */
        .idauOE{color:rgb(71,71,71);} .idauOE *{font-family:'Open Sans',sans-serif;}
        /* sc-component-id: Container-sc-xros6q */

        /* sc-component-id: AnimatedSections-sc-8n42n7 */

        /* sc-component-id: Sections-sc-76vzye */

        /* sc-component-id: Section-sc-qx3mro */

        /* sc-component-id: Container-sc-1v83ki3 */

        /* sc-component-id: Text-sc-os1l8v */

        /* sc-component-id: Div-sc-1xu70qp */

        /* sc-component-id: CollapseTitle-sc-15vfdf4 */

        /* sc-component-id: CollapseArrow-sc-suqy7n */

        /* sc-component-id: GearBackground-sc-w3mwoy */

        /* sc-component-id: Svg-sc-1tg0df8 */

        /* sc-component-id: FooterAliradarLogo-sc-23umav */

        /* sc-component-id: StyledFooterGear-sc-1t0eo26 */

        /* sc-component-id: StyledFooterLogo-sc-148xhmd */

        /* sc-component-id: Div-sc-g9jh15 */

        /* sc-component-id: Svg-sc-1i77m53 */

        /* sc-component-id: SectionTabContainer-sc-98gtzv */

        /* sc-component-id: SectionTabsContainer-sc-lvggt3 */

        /* sc-component-id: Div-sc-1gg063b */

        /* sc-component-id: TextRotatorText-sc-1sv0f4v */

        /* sc-component-id: TextRotatorTextDefault-sc-15fh1b1 */

        /* sc-component-id: TextRotatorTextHover-sc-noubvo */

        /* sc-component-id: Svg-sc-1bm3xd1 */

        /* sc-component-id: BellCircle-sc-1v9aldv */

        /* sc-component-id: BellPath-sc-nhxeqb */

        /* sc-component-id: Path-sc-1nrd8bf */

        /* sc-component-id: Div-sc-3bdiy3 */

        /* sc-component-id: Text-sc-11pgdr4 */

        /* sc-component-id: HeaderTitle-sc-1d5y2rz */

        /* sc-component-id: HeaderSubtitle-sc-1v2r4oy */

        /* sc-component-id: Div-sc-1serbk */

        /* sc-component-id: HeaderIcon-sc-1gvhcqr */

        /* sc-component-id: EmptyContainer-sc-nurzcw */

        /* sc-component-id: MorePhotos-sc-18a56zb */

        /* sc-component-id: SmallItemListContainer-sc-8unuc5 */

        /* sc-component-id: Image-sc-19574y7 */

        /* sc-component-id: SmallItemBG-sc-83wsec */

        /* sc-component-id: LoaderWrapper-sc-cmepm0 */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: LoaderImageWrapper-sc-ax4se2 */

        /* sc-component-id: Photos-sc-e3jdlw */

        /* sc-component-id: Heading-sc-1h3k8px */

        /* sc-component-id: HR-sc-eifmta */

        /* sc-component-id: A-sc-18o4bx9 */

        /* sc-component-id: SmallPhotoListLoader-sc-1dq5vny */

        /* sc-component-id: IndetermRatingSpan-sc-6dvehw */

        /* sc-component-id: SellerInfoWrapper-sc-km40o7 */

        /* sc-component-id: SellerInfoItem-sc-k5x5tw */

        /* sc-component-id: IndetermText-sc-iuejml */

        /* sc-component-id: LoaderWrapper-sc-4sjux8 */

        /* sc-component-id: SellerBody-sc-5lbigt */

        /* sc-component-id: SellerBodyIndeterm-sc-5ria4o */

        /* sc-component-id: LoaderWrapper-sc-t41xb5 */

        /* sc-component-id: ButtonWrapper-sc-quxp8a */

        /* sc-component-id: Quantity-sc-15shy4f */

        /* sc-component-id: Notice-sc-2b9hp */

        /* sc-component-id: PriceBody-sc-cq0r3n */

        /* sc-component-id: GraphWrapper-sc-wpci4x */

        /* sc-component-id: Button-sc-1nxdeu */

        /* sc-component-id: Version-sc-10s80ar */

        /* sc-component-id: Svg-sc-8hmfx2 */

        /* sc-component-id: SettingsTitle-sc-jatqet */

        /* sc-component-id: WidgetTabTitleWrapper-sc-1c90pxf */

        /* sc-component-id: CloseWrapper-sc-okguek */

        /* sc-component-id: SvgPointer-sc-cdic6s */

        /* sc-component-id: SettingHeader-sc-1lx5qe7 */

        /* sc-component-id: Button-sc-1ktvb9a */

        /* sc-component-id: Message-sc-13mcdn7 */

        /* sc-component-id: Section-sc-ybfhg1 */

        /* sc-component-id: Avatar-sc-114wmbl */

        /* sc-component-id: Image-sc-1cejwsf */

        /* sc-component-id: WrapperInner-sc-8cfck6 */

        /* sc-component-id: AuthAgent-sc-1n54s1r */

        /* sc-component-id: Info-sc-bsy65g */

        /* sc-component-id: Name-sc-10mko78 */

        /* sc-component-id: Social-sc-1aquwk9 */

        /* sc-component-id: Wrapper-sc-f3808k */

        /* sc-component-id: Logout-sc-1mdyu9r */

        /* sc-component-id: Checkmark-sc-1d3lzgi */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Input-sc-m9sty1 */

        /* sc-component-id: Checkbox-sc-1ldruyx */

        /* sc-component-id: Select-sc-1s8563s */

        /* sc-component-id: Arrow-sc-1vn723k */

        /* sc-component-id: ArrowWrapper-sc-7v70bt */

        /* sc-component-id: Field-sc-wwe1q8 */

        /* sc-component-id: Options-sc-fvgjv */

        /* sc-component-id: OptionsInner-sc-1rk6lu1 */

        /* sc-component-id: Option-sc-bk1780 */

        /* sc-component-id: Selected-sc-1obdp8e */

        /* sc-component-id: WrapperInner-sc-1xura5i */

        /* sc-component-id: CheckboxWrapper-sc-c507ex */

        /* sc-component-id: Label-sc-1tkknm4 */

        /* sc-component-id: SettingsItemOptionRow-sc-1vzlds9 */

        /* sc-component-id: SettingsItemSelectRow-sc-1tx362f */

        /* sc-component-id: SelectWrapper-sc-1ps5k3y */

        /* sc-component-id: ListOfSettings-sc-e3qghm */

        /* sc-component-id: Link-sc-ypt19z */

        /* sc-component-id: SettingsFooter-sc-1vydg8d */

        /* sc-component-id: ListWrapperFull-sc-1s19kzr */

        /* sc-component-id: Background-sc-1vs5i8o */

        /* sc-component-id: SettingsContent-sc-j0hefo */

        /* sc-component-id: AnimationWrapper-sc-1ig4im3 */

        /* sc-component-id: FirstPulse-sc-bc4pmm */

        /* sc-component-id: SecondPulse-sc-1oveg9l */

        /* sc-component-id: PulseCore-sc-5ydjpt */

        /* sc-component-id: FooterTextContainer-sc-14v6et1 */

        /* sc-component-id: ButtonContainer-sc-lx0wwb */

        /* sc-component-id: FooterTextWrapper-sc-d8zaof */

        /* sc-component-id: IconWrapper-sc-fk7goo */

        /* sc-component-id: BoardingFooter-sc-1ekyqpf */

        /* sc-component-id: TextHeader-sc-ufdnys */

        /* sc-component-id: TextBody-sc-lo51xq */

        /* sc-component-id: ButtonWrapper-sc-jvbnrf */

        /* sc-component-id: BoardingBody-sc-1o8yiwc */

        /* sc-component-id: TextWrapper-sc-1lm5pir */

        /* sc-component-id: BoardingWrapper-sc-1rve78f */

        /* sc-component-id: FirstButtonWrapper-sc-1b0zixm */

        /* sc-component-id: SecondButtonWrapper-sc-1vluc3v */

        /* sc-component-id: ImageWrapper-sc-s2vcri */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: HeaderWrapper-sc-1t1r5rl */

        /* sc-component-id: LastWidgetContainer-sc-1wrjzwk */

        /* sc-component-id: ButtonWrapper-sc-ftywqn */

        /* sc-component-id: ByeButtonsWrapper-sc-bytg21 */

        /* sc-component-id: LastWidgetWrapper-sc-oq1pem */

        /* sc-component-id: ArrowWrapper-sc-19obbn4 */

        /* sc-component-id: PathWrapper-sc-854cby */

        /* sc-component-id: IconWrapper-sc-1tnd975 */

        /* sc-component-id: BoardingWraper-sc-1s0ehc2 */

        /* sc-component-id: TooltipText-sc-k41toi */

        /* sc-component-id: CloseSectionWrapper-sc-1afd6ye */

        /* sc-component-id: BoardingOverlayStyled-sc-1yzfovk */

        /* sc-component-id: Inner-sc-vhuemm */

        /* sc-component-id: Bold-sc-svzrv7 */

        /* sc-component-id: SvgWrapper-sc-b3wm7w */

        /* sc-component-id: StarWrapper-sc-1rq7245 */

        /* sc-component-id: ReviewContainer-sc-th3ex9 */

        /* sc-component-id: ReviewFooter-sc-1020oit */

        /* sc-component-id: OrderInfoItem-sc-hp9pk5 */

        /* sc-component-id: ClosePanel-sc-roqeb2 */

        /* sc-component-id: Background-sc-1r7n543 */

        /* sc-component-id: GalleryWidgetContainer-sc-1hblxjz */

        /* sc-component-id: FullReview-sc-1bvi3gv */

        /* sc-component-id: ReviewPhoto-sc-1vklxsl */

        /* sc-component-id: Area-sc-1v31su8 */

        /* sc-component-id: LeftArea-sc-belnp3 */

        /* sc-component-id: RightCenterArea-sc-1rfyf9y */

        /* sc-component-id: CenterArea-sc-a6cnw7 */

        /* sc-component-id: RightArea-sc-kbdqz9 */

        /* sc-component-id: ReviewText-sc-abdq6w */

        /* sc-component-id: MyScrollArea-sc-2ylyk */

        /* sc-component-id: ReviewList-sc-17cdbz0 */

        /* sc-component-id: Svg-sc-pezk7g */

        /* sc-component-id: TabSpacer-sc-hu29vs */

        /* sc-component-id: LoaderWrapper-sc-t22m4a */

        /* sc-component-id: LoaderImageWrapper-sc-14j6m8s */

        /* sc-component-id: Div-sc-1y1h44y */

        /* sc-component-id: Div-sc-jz8v88 */

        /* sc-component-id: Div-sc-tdyl6u */

        /* sc-component-id: Svg-sc-qmb3nr */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Div-sc-1l0k4ov */

        /* sc-component-id: Div-sc-1xk0sa8 */

        /* sc-component-id: WidgetRight-sc-1vmzu66 */

        /* sc-component-id: Button-sc-y63xeu */

        /* sc-component-id: Bottom-sc-cxvu4r */

        /* sc-component-id: StyledWrapper-sc-dvb0ug */

        /* sc-component-id: StyledInner-sc-1v9w2bf */

        /* sc-component-id: SvgPointer-sc-a3u0hq */

        /* sc-component-id: RadarLogoWrapper-sc-usuvtu */

        /* sc-component-id: SvgPointer-sc-11efy2w */

        /* sc-component-id: MouseOverlay-sc-llmpeq */

        /* sc-component-id: Link-sc-bk1olv */

        /* sc-component-id: Item-sc-1x6h5rh */

        /* sc-component-id: ItemStars-sc-1h9ei4a */

        /* sc-component-id: ItemOrders-sc-tfn0bp */

        /* sc-component-id: ItemInfo-sc-1t6lwha */

        /* sc-component-id: ItemContent-sc-1w0qrbh */

        /* sc-component-id: ItemHeading-sc-1a3653y */

        /* sc-component-id: ItemSeller-sc-pafzvs */

        /* sc-component-id: ItemSellerStatus-sc-n2ox43 */

        /* sc-component-id: ItemSellerStatusNum-sc-ty3j5r */

        /* sc-component-id: Currency-sc-8rntl */

        /* sc-component-id: Tooltip-sc-14qsp59 */

        /* sc-component-id: Card-sc-15ny6o7 */

        /* sc-component-id: CardBody-sc-6amc4b */

        /* sc-component-id: BigItem-sc-chan3i */

        /* sc-component-id: Image-sc-7mjfls */

        /* sc-component-id: Text-sc-1i7w0js */

        /* sc-component-id: Heading-sc-1ba0bak */

        /* sc-component-id: Container-sc-f503fz */

        /* sc-component-id: Empty-sc-2eyi60 */

        /* sc-component-id: Loader-sc-nm2kwx */

        /* sc-component-id: Img-sc-1k58obe */

        /* sc-component-id: Svg-sc-1prycez */

        /* sc-component-id: Div-sc-1cujuhi */

        /* sc-component-id: Container-sc-14bpwj4 */

        /* sc-component-id: CustomLink-sc-1azkxn2 */

        /* sc-component-id: Icons-sc-1xk8t25 */

        /* sc-component-id: Image-sc-1tuey89 */

        /* sc-component-id: Rows-sc-1y3z2i5 */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Row-sc-o1s3mk */

        /* sc-component-id: Color-sc-98w3qw */

        /* sc-component-id: SignIn-sc-g5phqy */

        /* sc-component-id: Empty-sc-7wan1e */

        /* sc-component-id: Img-sc-1tttuew */

        /* sc-component-id: Div-sc-uk5kf */

        /* sc-component-id: WidgetMain-sc-y8bzbp */

        /* sc-component-id: WidgetFooter-sc-182uvcb */

        /* sc-component-id: WidgetHeader-sc-1q4t5cy */

        /* sc-component-id: FooterTab-sc-1wgw72z */

        /* sc-component-id: SideWidgetControls-sc-1kjod37 */

        /* sc-component-id: SvgPointer-sc-1jf4g2g */

        /* sc-component-id: SvgColor-sc-3ymkws */

        /* sc-component-id: SideWidgetMain-sc-sqs76f */

        /* sc-component-id: SideWidgetContainer-sc-rmpj8y */

        /* sc-component-id: SideAndBoarding-sc-36swhr */

        /* sc-component-id: AuthPopup-sc-iaojz9 */

        /* sc-component-id: AuthText-sc-b3kqgl */

        /* sc-component-id: ButtonWrapper-sc-3o2gwq */

        /* sc-component-id: ButtonWrapper-sc-wba817 */

        /* sc-component-id: Favorites-sc-gdrp7q */

        /* sc-component-id: TextWrapper-sc-1hbnb6o */

        /* sc-component-id: CloseIconWrapper-sc-1fktqfg */

        /* sc-component-id: AvailablePlatformsWrapper-sc-1hfi7dd */

        /* sc-component-id: FooterWrapper-sc-1euweht */

        /* sc-component-id: Svg-sc-c2r6gp */

        /* sc-component-id: PlatformLinks-sc-dk3tiz */

        /* sc-component-id: IconWithMargin-sc-17ufa41 */

        /* sc-component-id: StyledImg-sc-1g9gbnb */

        /* sc-component-id: LogoutWrapper-sc-g6tai0 */

        /* sc-component-id: DefaultAvatarText-sc-3lrmeu */

        /* sc-component-id: DefaultAvatarWrapper-sc-1wve8vb */

        /* sc-component-id: Avatar-sc-2xiqme */

        /* sc-component-id: HeaderPopup-sc-1o4o3wy */

        /* sc-component-id: ArrowIconWrapper-sc-1h2y0s2 */

        /* sc-component-id: SidePopupToggler-sc-1g0u53p */

        /* sc-component-id: RadarLogoWrapper-sc-159zeos */

        /* sc-component-id: SupportLogoWrapper-sc-hzpw34 */

        /* sc-component-id: SvgPointer-sc-lw9y6n */

        /* sc-component-id: Discount-sc-163u1q */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Delivery-sc-yjk3zv */

        /* sc-component-id: BeruWrapper-sc-1ey1u28 */

        /* sc-component-id: DiscountText-sc-1381ckw */

        /* sc-component-id: ItemWrapper-sc-147c1r4 */

        /* sc-component-id: ImageWrapper-sc-1op80ta */

        /* sc-component-id: TextWrapper-sc-9awx6n */

        /* sc-component-id: ItemImage-sc-yqd006 */

        /* sc-component-id: ItemCurrency-sc-oqi2ev */

        /* sc-component-id: Skeleton-sc-nl26ke */

        /* sc-component-id: ItemListWrapper-sc-d5343s */

        /* sc-component-id: ItemListInnerWrapper-sc-871edi */

        /* sc-component-id: HotCategories-sc-1joja06 */

        /* sc-component-id: CategoriesWrapperInner-sc-u252oq */

        /* sc-component-id: CategoryItem-sc-i756pc */

        /* sc-component-id: ArrowWrapper-sc-6zuv42 */

        /* sc-component-id: CloseIconWrapper-sc-ajoofq */

        /* sc-component-id: HeaderHotProducts-sc-1vb35a */

        /* sc-component-id: HotIconWrapper-sc-re18y1 */

        /* sc-component-id: HotTitle-sc-1k0jp02 */

        /* sc-component-id: SeeAllButton-sc-1eqsieh */

        /* sc-component-id: HotProducts-sc-1qh8d4u */

        /* sc-component-id: Blur-sc-4aha0x */

        /* sc-component-id: InputPlaceholderUpper-sc-1pmfks3 */

        /* sc-component-id: InputPlaceholder-sc-d7b5dy */

        /* sc-component-id: StyledPath-sc-hty0rt */

        /* sc-component-id: IconWrapper-sc-1c1daye */

        /* sc-component-id: InputWrapper-sc-2lv703 */

        /* sc-component-id: LabelWrapper-sc-1o6unj */

        /* sc-component-id: FirstInputWrapper-sc-ph6m7s */

        /* sc-component-id: InputFieldWrapper-sc-biy8lf */

        /* sc-component-id: InsturctionWrapper-sc-2vmqp0 */

        /* sc-component-id: InsturctionQuestion-sc-1ah9ngs */

        /* sc-component-id: InsturctionButtonWrapper-sc-1g0gfqe */

        /* sc-component-id: SkipButton-sc-zsgo9e */

        /* sc-component-id: InsturctionButton-sc-vm7xbn */

        /* sc-component-id: DropdownContainer-sc-1jp0d0a */

        /* sc-component-id: DropdownWrapper1-sc-1r2cnda */

        /* sc-component-id: DropdownWrapper2-sc-1d9gm9q */

        /* sc-component-id: DropdownItem-sc-k8l5bh */

        /* sc-component-id: ShopLinksWrapper-sc-8frumo */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: ShopLinksTitle-sc-1pmel3w */

        /* sc-component-id: ButtonWrapper-sc-17bxlvv */

        /* sc-component-id: ButtonsWrapper-sc-1f3v8rh */

        /* sc-component-id: StyledButton-sc-g5o3de */

        /* sc-component-id: ScrolableWrapper-sc-ce8avx */

        /* sc-component-id: BasePopup-sc-1eyuxop */

        /* sc-component-id: Text-sc-1c9hfr0 */

        /* sc-component-id: Div-sc-a1ifnu */

        /* sc-component-id: SectionName-sc-12bw2py */

        /* sc-component-id: Badge-sc-1om1z5d */

        /* sc-component-id: ItemInfo-sc-1lqptu0 */

        /* sc-component-id: Detail-sc-19c2h7e */

        /* sc-component-id: DetailBottom-sc-yw18lz */

        /* sc-component-id: DetailIcon-sc-11kck9j */

        /* sc-component-id: DetailTitle-sc-2v11lg */

        /* sc-component-id: DetailTop-sc-5slzsx */

        /* sc-component-id: Title-sc-1dme73c */

        /* sc-component-id: Currency-sc-1uhtz0s */

        /* sc-component-id: ItemPrice-sc-12qdagw */

        /* sc-component-id: ItemTooltip-sc-1j3sjsn */

        /* sc-component-id: Image-sc-vtph16 */

        /* sc-component-id: LinkContainer-sc-1c3f6ks */

        /* sc-component-id: PriceAndBadge-sc-1d3qr2 */

        /* sc-component-id: Sticker-sc-j7p93s */

        /* sc-component-id: Text-sc-1v04cvr */

        /* sc-component-id: TrashCanIconWrapper-sc-q5shyw */

        /* sc-component-id: Item-sc-e57v6w */

        /* sc-component-id: Items-sc-ppthrc */

        /* sc-component-id: Root-sc-1dbdpge */

        /* sc-component-id: Empty-sc-rf5qf5 */

        /* sc-component-id: Loader-sc-16dgune */

        /* sc-component-id: Img-sc-3qiyq9 */

        /* sc-component-id: Icon-sc-1h5nrg6 */

        /* sc-component-id: Button-sc-7upep9 */

        /* sc-component-id: Currency-sc-nhlc1y */

        /* sc-component-id: Image-sc-gbdwj8 */

        /* sc-component-id: Name-sc-trg484 */

        /* sc-component-id: Price-sc-1crb62u */

        /* sc-component-id: PriceAndButton-sc-16421fb */

        /* sc-component-id: HeaderItem-sc-2pyx3f */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Name-sc-wzzz73 */

        /* sc-component-id: Text-sc-1fni2wx */

        /* sc-component-id: Title-sc-1ycsb1h */

        /* sc-component-id: Root-sc-e0ikxb */

        /* sc-component-id: Icon-sc-1vjn8hh */

        /* sc-component-id: Subtitle-sc-xk2got */

        /* sc-component-id: Text-sc-5pr18 */

        /* sc-component-id: Title-sc-17410d2 */

        /* sc-component-id: SectionHeader-sc-rihplt */

        /* sc-component-id: Root-sc-1tj69a4 */

        /* sc-component-id: Root-sc-37oe2p */

        /* sc-component-id: ItemsSlot-sc-1hqbn77 */

        /* sc-component-id: SignIn-sc-uxx0bw */

        /* sc-component-id: TabsSlot-sc-1ogo3xg */

        /* sc-component-id: CategoriesSlot-sc-6nxiqx */

        /* sc-component-id: Icon-sc-1witxe4 */

        /* sc-component-id: ItemsSlot-sc-nzyv8r */

        /* sc-component-id: OpenFull-sc-1twpbnd */

        /* sc-component-id: LoaderWrapper-sc-1c3cdmg */

        /* sc-component-id: LoaderImageWrapper-sc-sqz0ak */

        /* sc-component-id: Cols-sc-ob4142 */

        /* sc-component-id: Col-sc-1yfliue */

        /* sc-component-id: ProductPopup-sc-ms2e4k */

        /* sc-component-id: FooterTabs-sc-5q6gsn */

        /* sc-component-id: FooterTab-sc-wr58nf */

        /* sc-component-id: Main-sc-n7fgwa */

        /* sc-component-id: SidePopup-sc-qmv31h */

        /* sc-component-id: SidePopupWrapper-sc-10ddhn4 */

        /* sc-component-id: Button-sc-1ph7ubl */

        /* sc-component-id: WrapperDefault-sc-luqo4s */

        /* sc-component-id: sc-global-3953266849 */

        /* sc-component-id: Wrapper-sc-j4z9n2 */

        /* sc-component-id: Subtitle-sc-1h3l84k */

        /* sc-component-id: FeatureTitle-sc-126vyxy */

        /* sc-component-id: Arrow-sc-28oo */

        /* sc-component-id: Section-sc-v272vk */

        /* sc-component-id: Text-sc-7ew18p */

        /* sc-component-id: Wrapper-sc-101d8xq */

        /* sc-component-id: Support-sc-o4lyvp */

        /* sc-component-id: Title-sc-uveyvi */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: Ready-sc-9v48u3 */

        /* sc-component-id: Wrapper-sc-1id0mjk */

        /* sc-component-id: Arrow-sc-ukpt7h */

        /* sc-component-id: IconsBlock-sc-1cw9dv7 */

        /* sc-component-id: IconBlock-sc-g3baw2 */

        /* sc-component-id: SectionBottom-sc-15bj3w3 */

        /* sc-component-id: SectionBottomText-sc-1t519v7 */

        /* sc-component-id: SectionFeature-sc-11tbviu */

        /* sc-component-id: SectionFeatureTitle-sc-e5kwqk */

        /* sc-component-id: SectionIcon-sc-hm5urn */

        /* sc-component-id: SectionIconText-sc-esa8i0 */

        /* sc-component-id: SectionIconTitle-sc-1qwb5zq */

        /* sc-component-id: SectionTop-sc-blobwh */

        /* sc-component-id: Wrapper-sc-1wyehb8 */

        /* sc-component-id: SectionPinnedExtension-sc-lfnz7 */

        /* sc-component-id: Hr-sc-jpgu5j */

        /* sc-component-id: SectionPinnedText-sc-kad4jm */

        /* sc-component-id: PopupSimulator-sc-snbgq4 */

        /* sc-component-id: StyledCloseButtonBlock-sc-bz6whi */

        /* sc-component-id: ByRadar-sc-14mif0w */

        /* sc-component-id: Checked-sc-1v1loo7 */

        /* sc-component-id: StyledRadarLogo-sc-1buonix */

        /* sc-component-id: ErrorText-sc-xvbdq9 */

        /* sc-component-id: sc-global-1622337232 */

        /* sc-component-id: StyledSeller-sc-1nw5e9c */

        /* sc-component-id: Title-sc-zai17u */

        /* sc-component-id: Icon-sc-arcd5f */

        /* sc-component-id: Text-sc-1az49qn */

        /* sc-component-id: PriceInfo-sc-2skoon */

        /* sc-component-id: PriceSection-sc-p2nqep */

        /* sc-component-id: StyledPriceText-sc-1tf23mt */

        /* sc-component-id: StyledPriceChangePercentAbsText-sc-11n03gq */

        /* sc-component-id: OpenProductButton-sc-jsnbau */

        /* sc-component-id: SimilarButton-sc-1sy80v3 */

        /* sc-component-id: StyledLeftBlock-sc-y9egtx */

        /* sc-component-id: StyledRightBlock-sc-b59jjc */

        /* sc-component-id: StyledSimilar-sc-e3iq3d */

        /* sc-component-id: StyledSimilarItem-sc-1kpzwti */

        /* sc-component-id: StyledSimilarPriceContainer-sc-1tb8vnp */

        /* sc-component-id: StyledSimilarPrice-sc-1e78ho9 */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: StyledSimilarImgBlock-sc-tpkw39 */

        /* sc-component-id: StyledAppBarGear-sc-2v9mm4 */

        /* sc-component-id: Left-sc-2ye29r */

        /* sc-component-id: Right-sc-7kjll8 */

        /* sc-component-id: ImgBlock-sc-e8b93 */

        /* sc-component-id: StyledInfo-sc-ch0og9 */

        /* sc-component-id: StyledInfoBody-sc-rf916l */

        /* sc-component-id: StyledAppBar-sc-15k9e7i */

        /* sc-component-id: StyledAppBarButtons-sc-a68wwo */

        /* sc-component-id: StyledFooterButtons-sc-18fxzy */

        /* sc-component-id: Text-sc-1c6lmqh */

        /* sc-component-id: VerticalLine-sc-1lr6wyj */

        /* sc-component-id: StyledProductRaiting-sc-1513c11 */

        /* sc-component-id: StyledProductInfo-sc-5k47mo */

        /* sc-component-id: StyledProductRaitingText-sc-x2yj9k */

        /* sc-component-id: StyledProduct-sc-1wblwz */

        /* sc-component-id: StyledProductItem-sc-19h4mk */

        /* sc-component-id: StyledProductInfoText-sc-1msvxwz */

        /* sc-component-id: StartButton-sc-1rrz552 */

        /* sc-component-id: OutsideClick-sc-1oz7raa */

        /* sc-component-id: HeaderWrapper-sc-tgpodq */

        /* sc-component-id: MoreInfo-sc-1tm87nt */

        /* sc-component-id: HeaderDescription-sc-1hspgw5 */

        /* sc-component-id: TextRed-sc-8e9rmg */

        /* sc-component-id: TextYellow-sc-15yn6oy */

        /* sc-component-id: ItemListWrapper-sc-j0geax */

        /* sc-component-id: ItemListInnerWrapper-sc-qurszg */

        /* sc-component-id: Svg-sc-6i0nsb */

        /* sc-component-id: StyledButton-sc-1f6mvvn */

        /* sc-component-id: ButtonWrapper-sc-10hhd7j */

        /* sc-component-id: HighlightedButton-sc-1g51513 */

        /* sc-component-id: TransparentButton-sc-1nywlqq */

        /* sc-component-id: QuestionSpan-sc-b8bkfo */

        /* sc-component-id: GearIconWrapper-sc-xj0v61 */

        /* sc-component-id: Svg-sc-q5ydn0 */

        /* sc-component-id: OpenButton-sc-18zek9s */

        /* sc-component-id: ArrowWrapper-sc-1u3jukd */

        /* sc-component-id: GearWrapper-sc-1e357l2 */

        /* sc-component-id: IconWrapper-sc-1jam84s */

        /* sc-component-id: TextWrapper-sc-rtsv6z */
    </style><style data-styled="" data-styled-version="4.4.1">
        /* sc-component-id: LogoWrapper-sc-iiv1hy */

        /* sc-component-id: FooterWrapper-sc-1nvvoev */

        /* sc-component-id: ArrowWrapper-sc-1qp8qou */

        /* sc-component-id: CategoriesWrapper-sc-114qp79 */

        /* sc-component-id: CategoriesWrapperInner-sc-gzcluy */

        /* sc-component-id: CategoryItem-sc-ugbvwm */

        /* sc-component-id: CategoryMore-sc-1a6wu5g */

        /* sc-component-id: CloseIconWrapper-sc-ilhe7y */

        /* sc-component-id: ItemInfoWrapper-sc-x8jmkb */

        /* sc-component-id: ItemInfoText-sc-x2sh23 */

        /* sc-component-id: ItemInfoCurrency-sc-1rshy01 */

        /* sc-component-id: ItemInfoName-sc-1e1u35o */

        /* sc-component-id: ItemInfoImageWraper-sc-1oqkum0 */

        /* sc-component-id: Widget-sc-1xld1yt */

        /* sc-component-id: WidgetWrapper-sc-xin3g6 */
    </style><style type="text/css">.apexcharts-canvas {
            position: relative;
            user-select: none;
            /* cannot give overflow: hidden as it will crop tooltips which overflow outside chart area */
        }


        /* scrollbar is not visible by default for legend, hence forcing the visibility */
        .apexcharts-canvas ::-webkit-scrollbar {
            -webkit-appearance: none;
            width: 6px;
        }

        .apexcharts-canvas ::-webkit-scrollbar-thumb {
            border-radius: 4px;
            background-color: rgba(0, 0, 0, .5);
            box-shadow: 0 0 1px rgba(255, 255, 255, .5);
            -webkit-box-shadow: 0 0 1px rgba(255, 255, 255, .5);
        }


        .apexcharts-inner {
            position: relative;
        }

        .apexcharts-text tspan {
            font-family: inherit;
        }

        .legend-mouseover-inactive {
            transition: 0.15s ease all;
            opacity: 0.20;
        }

        .apexcharts-series-collapsed {
            opacity: 0;
        }

        .apexcharts-tooltip {
            border-radius: 5px;
            box-shadow: 2px 2px 6px -4px #999;
            cursor: default;
            font-size: 14px;
            left: 62px;
            opacity: 0;
            pointer-events: none;
            position: absolute;
            top: 20px;
            display: flex;
            flex-direction: column;
            overflow: hidden;
            white-space: nowrap;
            z-index: 12;
            transition: 0.15s ease all;
        }

        .apexcharts-tooltip.apexcharts-active {
            opacity: 1;
            transition: 0.15s ease all;
        }

        .apexcharts-tooltip.apexcharts-theme-light {
            border: 1px solid #e3e3e3;
            background: rgba(255, 255, 255, 0.96);
        }

        .apexcharts-tooltip.apexcharts-theme-dark {
            color: #fff;
            background: rgba(30, 30, 30, 0.8);
        }

        .apexcharts-tooltip * {
            font-family: inherit;
        }


        .apexcharts-tooltip-title {
            padding: 6px;
            font-size: 15px;
            margin-bottom: 4px;
        }

        .apexcharts-tooltip.apexcharts-theme-light .apexcharts-tooltip-title {
            background: #ECEFF1;
            border-bottom: 1px solid #ddd;
        }

        .apexcharts-tooltip.apexcharts-theme-dark .apexcharts-tooltip-title {
            background: rgba(0, 0, 0, 0.7);
            border-bottom: 1px solid #333;
        }

        .apexcharts-tooltip-text-value,
        .apexcharts-tooltip-text-z-value {
            display: inline-block;
            font-weight: 600;
            margin-left: 5px;
        }

        .apexcharts-tooltip-text-z-label:empty,
        .apexcharts-tooltip-text-z-value:empty {
            display: none;
        }

        .apexcharts-tooltip-text-value,
        .apexcharts-tooltip-text-z-value {
            font-weight: 600;
        }

        .apexcharts-tooltip-marker {
            width: 12px;
            height: 12px;
            position: relative;
            top: 0px;
            margin-right: 10px;
            border-radius: 50%;
        }

        .apexcharts-tooltip-series-group {
            padding: 0 10px;
            display: none;
            text-align: left;
            justify-content: left;
            align-items: center;
        }

        .apexcharts-tooltip-series-group.apexcharts-active .apexcharts-tooltip-marker {
            opacity: 1;
        }

        .apexcharts-tooltip-series-group.apexcharts-active,
        .apexcharts-tooltip-series-group:last-child {
            padding-bottom: 4px;
        }

        .apexcharts-tooltip-series-group-hidden {
            opacity: 0;
            height: 0;
            line-height: 0;
            padding: 0 !important;
        }

        .apexcharts-tooltip-y-group {
            padding: 6px 0 5px;
        }

        .apexcharts-tooltip-box, .apexcharts-custom-tooltip {
            padding: 4px 8px;
        }

        .apexcharts-tooltip-boxPlot {
            display: flex;
            flex-direction: column-reverse;
        }

        .apexcharts-tooltip-box>div {
            margin: 4px 0;
        }

        .apexcharts-tooltip-box span.value {
            font-weight: bold;
        }

        .apexcharts-tooltip-rangebar {
            padding: 5px 8px;
        }

        .apexcharts-tooltip-rangebar .category {
            font-weight: 600;
            color: #777;
        }

        .apexcharts-tooltip-rangebar .series-name {
            font-weight: bold;
            display: block;
            margin-bottom: 5px;
        }

        .apexcharts-xaxistooltip {
            opacity: 0;
            padding: 9px 10px;
            pointer-events: none;
            color: #373d3f;
            font-size: 13px;
            text-align: center;
            border-radius: 2px;
            position: absolute;
            z-index: 10;
            background: #ECEFF1;
            border: 1px solid #90A4AE;
            transition: 0.15s ease all;
        }

        .apexcharts-xaxistooltip.apexcharts-theme-dark {
            background: rgba(0, 0, 0, 0.7);
            border: 1px solid rgba(0, 0, 0, 0.5);
            color: #fff;
        }

        .apexcharts-xaxistooltip:after,
        .apexcharts-xaxistooltip:before {
            left: 50%;
            border: solid transparent;
            content: " ";
            height: 0;
            width: 0;
            position: absolute;
            pointer-events: none;
        }

        .apexcharts-xaxistooltip:after {
            border-color: rgba(236, 239, 241, 0);
            border-width: 6px;
            margin-left: -6px;
        }

        .apexcharts-xaxistooltip:before {
            border-color: rgba(144, 164, 174, 0);
            border-width: 7px;
            margin-left: -7px;
        }

        .apexcharts-xaxistooltip-bottom:after,
        .apexcharts-xaxistooltip-bottom:before {
            bottom: 100%;
        }

        .apexcharts-xaxistooltip-top:after,
        .apexcharts-xaxistooltip-top:before {
            top: 100%;
        }

        .apexcharts-xaxistooltip-bottom:after {
            border-bottom-color: #ECEFF1;
        }

        .apexcharts-xaxistooltip-bottom:before {
            border-bottom-color: #90A4AE;
        }

        .apexcharts-xaxistooltip-bottom.apexcharts-theme-dark:after {
            border-bottom-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-xaxistooltip-bottom.apexcharts-theme-dark:before {
            border-bottom-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-xaxistooltip-top:after {
            border-top-color: #ECEFF1
        }

        .apexcharts-xaxistooltip-top:before {
            border-top-color: #90A4AE;
        }

        .apexcharts-xaxistooltip-top.apexcharts-theme-dark:after {
            border-top-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-xaxistooltip-top.apexcharts-theme-dark:before {
            border-top-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-xaxistooltip.apexcharts-active {
            opacity: 1;
            transition: 0.15s ease all;
        }

        .apexcharts-yaxistooltip {
            opacity: 0;
            padding: 4px 10px;
            pointer-events: none;
            color: #373d3f;
            font-size: 13px;
            text-align: center;
            border-radius: 2px;
            position: absolute;
            z-index: 10;
            background: #ECEFF1;
            border: 1px solid #90A4AE;
        }

        .apexcharts-yaxistooltip.apexcharts-theme-dark {
            background: rgba(0, 0, 0, 0.7);
            border: 1px solid rgba(0, 0, 0, 0.5);
            color: #fff;
        }

        .apexcharts-yaxistooltip:after,
        .apexcharts-yaxistooltip:before {
            top: 50%;
            border: solid transparent;
            content: " ";
            height: 0;
            width: 0;
            position: absolute;
            pointer-events: none;
        }

        .apexcharts-yaxistooltip:after {
            border-color: rgba(236, 239, 241, 0);
            border-width: 6px;
            margin-top: -6px;
        }

        .apexcharts-yaxistooltip:before {
            border-color: rgba(144, 164, 174, 0);
            border-width: 7px;
            margin-top: -7px;
        }

        .apexcharts-yaxistooltip-left:after,
        .apexcharts-yaxistooltip-left:before {
            left: 100%;
        }

        .apexcharts-yaxistooltip-right:after,
        .apexcharts-yaxistooltip-right:before {
            right: 100%;
        }

        .apexcharts-yaxistooltip-left:after {
            border-left-color: #ECEFF1;
        }

        .apexcharts-yaxistooltip-left:before {
            border-left-color: #90A4AE;
        }

        .apexcharts-yaxistooltip-left.apexcharts-theme-dark:after {
            border-left-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-yaxistooltip-left.apexcharts-theme-dark:before {
            border-left-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-yaxistooltip-right:after {
            border-right-color: #ECEFF1;
        }

        .apexcharts-yaxistooltip-right:before {
            border-right-color: #90A4AE;
        }

        .apexcharts-yaxistooltip-right.apexcharts-theme-dark:after {
            border-right-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-yaxistooltip-right.apexcharts-theme-dark:before {
            border-right-color: rgba(0, 0, 0, 0.5);
        }

        .apexcharts-yaxistooltip.apexcharts-active {
            opacity: 1;
        }

        .apexcharts-yaxistooltip-hidden {
            display: none;
        }

        .apexcharts-xcrosshairs,
        .apexcharts-ycrosshairs {
            pointer-events: none;
            opacity: 0;
            transition: 0.15s ease all;
        }

        .apexcharts-xcrosshairs.apexcharts-active,
        .apexcharts-ycrosshairs.apexcharts-active {
            opacity: 1;
            transition: 0.15s ease all;
        }

        .apexcharts-ycrosshairs-hidden {
            opacity: 0;
        }

        .apexcharts-selection-rect {
            cursor: move;
        }

        .svg_select_boundingRect, .svg_select_points_rot {
            pointer-events: none;
            opacity: 0;
            visibility: hidden;
        }
        .apexcharts-selection-rect + g .svg_select_boundingRect,
        .apexcharts-selection-rect + g .svg_select_points_rot {
            opacity: 0;
            visibility: hidden;
        }

        .apexcharts-selection-rect + g .svg_select_points_l,
        .apexcharts-selection-rect + g .svg_select_points_r {
            cursor: ew-resize;
            opacity: 1;
            visibility: visible;
        }

        .svg_select_points {
            fill: #efefef;
            stroke: #333;
            rx: 2;
        }

        .apexcharts-svg.apexcharts-zoomable.hovering-zoom {
            cursor: crosshair
        }

        .apexcharts-svg.apexcharts-zoomable.hovering-pan {
            cursor: move
        }

        .apexcharts-zoom-icon,
        .apexcharts-zoomin-icon,
        .apexcharts-zoomout-icon,
        .apexcharts-reset-icon,
        .apexcharts-pan-icon,
        .apexcharts-selection-icon,
        .apexcharts-menu-icon,
        .apexcharts-toolbar-custom-icon {
            cursor: pointer;
            width: 20px;
            height: 20px;
            line-height: 24px;
            color: #6E8192;
            text-align: center;
        }

        .apexcharts-zoom-icon svg,
        .apexcharts-zoomin-icon svg,
        .apexcharts-zoomout-icon svg,
        .apexcharts-reset-icon svg,
        .apexcharts-menu-icon svg {
            fill: #6E8192;
        }

        .apexcharts-selection-icon svg {
            fill: #444;
            transform: scale(0.76)
        }

        .apexcharts-theme-dark .apexcharts-zoom-icon svg,
        .apexcharts-theme-dark .apexcharts-zoomin-icon svg,
        .apexcharts-theme-dark .apexcharts-zoomout-icon svg,
        .apexcharts-theme-dark .apexcharts-reset-icon svg,
        .apexcharts-theme-dark .apexcharts-pan-icon svg,
        .apexcharts-theme-dark .apexcharts-selection-icon svg,
        .apexcharts-theme-dark .apexcharts-menu-icon svg,
        .apexcharts-theme-dark .apexcharts-toolbar-custom-icon svg {
            fill: #f3f4f5;
        }

        .apexcharts-canvas .apexcharts-zoom-icon.apexcharts-selected svg,
        .apexcharts-canvas .apexcharts-selection-icon.apexcharts-selected svg,
        .apexcharts-canvas .apexcharts-reset-zoom-icon.apexcharts-selected svg {
            fill: #008FFB;
        }

        .apexcharts-theme-light .apexcharts-selection-icon:not(.apexcharts-selected):hover svg,
        .apexcharts-theme-light .apexcharts-zoom-icon:not(.apexcharts-selected):hover svg,
        .apexcharts-theme-light .apexcharts-zoomin-icon:hover svg,
        .apexcharts-theme-light .apexcharts-zoomout-icon:hover svg,
        .apexcharts-theme-light .apexcharts-reset-icon:hover svg,
        .apexcharts-theme-light .apexcharts-menu-icon:hover svg {
            fill: #333;
        }

        .apexcharts-selection-icon,
        .apexcharts-menu-icon {
            position: relative;
        }

        .apexcharts-reset-icon {
            margin-left: 5px;
        }

        .apexcharts-zoom-icon,
        .apexcharts-reset-icon,
        .apexcharts-menu-icon {
            transform: scale(0.85);
        }

        .apexcharts-zoomin-icon,
        .apexcharts-zoomout-icon {
            transform: scale(0.7)
        }

        .apexcharts-zoomout-icon {
            margin-right: 3px;
        }

        .apexcharts-pan-icon {
            transform: scale(0.62);
            position: relative;
            left: 1px;
            top: 0px;
        }

        .apexcharts-pan-icon svg {
            fill: #fff;
            stroke: #6E8192;
            stroke-width: 2;
        }

        .apexcharts-pan-icon.apexcharts-selected svg {
            stroke: #008FFB;
        }

        .apexcharts-pan-icon:not(.apexcharts-selected):hover svg {
            stroke: #333;
        }

        .apexcharts-toolbar {
            position: absolute;
            z-index: 11;
            max-width: 176px;
            text-align: right;
            border-radius: 3px;
            padding: 0px 6px 2px 6px;
            display: flex;
            justify-content: space-between;
            align-items: center;
        }

        .apexcharts-menu {
            background: #fff;
            position: absolute;
            top: 100%;
            border: 1px solid #ddd;
            border-radius: 3px;
            padding: 3px;
            right: 10px;
            opacity: 0;
            min-width: 110px;
            transition: 0.15s ease all;
            pointer-events: none;
        }

        .apexcharts-menu.apexcharts-menu-open {
            opacity: 1;
            pointer-events: all;
            transition: 0.15s ease all;
        }

        .apexcharts-menu-item {
            padding: 6px 7px;
            font-size: 12px;
            cursor: pointer;
        }

        .apexcharts-theme-light .apexcharts-menu-item:hover {
            background: #eee;
        }

        .apexcharts-theme-dark .apexcharts-menu {
            background: rgba(0, 0, 0, 0.7);
            color: #fff;
        }

        @media screen and (min-width: 768px) {
            .apexcharts-canvas:hover .apexcharts-toolbar {
                opacity: 1;
            }
        }

        .apexcharts-datalabel.apexcharts-element-hidden {
            opacity: 0;
        }

        .apexcharts-pie-label,
        .apexcharts-datalabels,
        .apexcharts-datalabel,
        .apexcharts-datalabel-label,
        .apexcharts-datalabel-value {
            cursor: default;
            pointer-events: none;
        }

        .apexcharts-pie-label-delay {
            opacity: 0;
            animation-name: opaque;
            animation-duration: 0.3s;
            animation-fill-mode: forwards;
            animation-timing-function: ease;
        }

        .apexcharts-canvas .apexcharts-element-hidden {
            opacity: 0;
        }

        .apexcharts-hide .apexcharts-series-points {
            opacity: 0;
        }

        .apexcharts-gridline,
        .apexcharts-annotation-rect,
        .apexcharts-tooltip .apexcharts-marker,
        .apexcharts-area-series .apexcharts-area,
        .apexcharts-line,
        .apexcharts-zoom-rect,
        .apexcharts-toolbar svg,
        .apexcharts-area-series .apexcharts-series-markers .apexcharts-marker.no-pointer-events,
        .apexcharts-line-series .apexcharts-series-markers .apexcharts-marker.no-pointer-events,
        .apexcharts-radar-series path,
        .apexcharts-radar-series polygon {
            pointer-events: none;
        }


        /* markers */

        .apexcharts-marker {
            transition: 0.15s ease all;
        }

        @keyframes opaque {
            0% {
                opacity: 0;
            }
            100% {
                opacity: 1;
            }
        }


        /* Resize generated styles */

        @keyframes resizeanim {
            from {
                opacity: 0;
            }
            to {
                opacity: 0;
            }
        }

        .resize-triggers {
            animation: 1ms resizeanim;
            visibility: hidden;
            opacity: 0;
        }

        .resize-triggers,
        .resize-triggers>div,
        .contract-trigger:before {
            content: " ";
            display: block;
            position: absolute;
            top: 0;
            left: 0;
            height: 100%;
            width: 100%;
            overflow: hidden;
        }

        .resize-triggers>div {
            background: #eee;
            overflow: auto;
        }

        .contract-trigger:before {
            width: 200%;
            height: 200%;
        }</style><style type="text/css">/* Copyright 2014-present Evernote Corporation. All rights reserved. */
        @keyframes caretBlink {
            from { opacity: 1.0; }
            to { opacity: 0.0; }
        }

        @keyframes rotateSpinner {
            from {
                transform:rotate(0deg);
            }
            to {
                transform:rotate(360deg);
            }
        }

        #text-tool-caret {
            animation-name: caretBlink;
            animation-iteration-count: infinite;
            animation-timing-function: cubic-bezier(1.0,0,0,1.0);
            animation-duration: 1s;
        }

        #en-markup-loading-spinner {
            position: absolute;
            top: calc(50% - 16px);
            left: calc(50% - 16px);
            width: 32px;
            height: 32px;
        }

        #en-markup-loading-spinner img {
            position: relative;
            top: 0px;
            left: 0px;
            animation-name: rotateSpinner;
            animation-duration: 0.6s;
            animation-iteration-count: infinite;
            animation-timing-function: linear;
        }
    </style><style type="text/css">/* Copyright 2014-present Evernote Corporation. All rights reserved. */
        .skitchToastBoxContainer {
            position: absolute;
            width: 100%;
            text-align: center;
            top: 30px;
            -webkit-user-select: none;
            -moz-user-select: none;
            pointer-events: none;
        }

        .skitchToastBox {
            width: 200px;
            height: 16px;
            padding: 12px;
            background-color: rgba(47, 55, 61, 0.95);
            border-radius: 4px;
            color: white;
            cursor: default;
            font-size: 10pt;
            text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.32);
            font-family: 'Soleil', Helvetica, Arial, sans-serif;
            border: 2px rgba(255, 255, 255, 0.38) solid;
        }

        .lang-zh-cn .skitchToastBox {
            font-family: '微软雅黑', 'Microsoft YaHei', SimSun,
            '&#x30E1;&#x30A4;&#x30EA;&#x30AA;', Meiryo, 'MS PGothic', 'Soleil',
            Helvetica, Arial, sans-serif;
        }

        .lang-ja-jp .skitchToastBox {
            font-family: '&#x30E1;&#x30A4;&#x30EA;&#x30AA;', Meiryo, 'MS PGothic',
            '微软雅黑', 'Microsoft YaHei', SimSun, 'Soleil', Helvetica, Arial,
            sans-serif;
        }

        .skitchToast {
            padding-left: 20px;
            padding-right: 20px;
            display: inline-block;
            height: 10px;
            color: #f1f5f8;
            text-align: center;
        }

        .skitchVisible {
            /* Don't remove this class it's a hack used by the Evernote Clipper */
        }
    </style><style type="text/css">/* Copyright 2014-present Evernote Corporation. All rights reserved. */

        @font-face {
            font-family: 'Soleil';
            src: url(data:application/font-woff2;base64,);
            font-weight: normal;
            font-style: normal;
        }
    </style><style type="text/css">/* Copyright 2019-present Evernote Corporation. All rights reserved. */

        #en-markup-disabled {
            position: fixed;
            z-index: 9999;
            width: 100%;
            height: 100%;
            top: 0px;
            left: 0px;
            cursor: default;
            -webkit-user-select: none;
        }

        #en-markup-alert-container {
            position: absolute;
            z-index: 9999;
            width: 450px;
            left: calc(50% - 225px);
            top: calc(50% - 85px);
            background-color: white;
            box-shadow: 0 2px 7px 1px rgba(0,0,0,0.35);
            -webkit-user-select: none;
        }

        #en-markup-alert-container .cell-1 {
            position: relative;
            height: 110px;
            width: 105px;
            float: left;
            text-align: center;
            background-image: url(data:image/png;base64,);
            background-position: 65% 50%;
            background-repeat: no-repeat;
        }

        #en-markup-alert-container .cell-2 {
            position: relative;
            float: left;
            width: 345px;
            margin-top: 29px;
            margin-bottom: 20px;
        }

        #en-markup-alert-container .cell-2 .cell-2-title {
            margin-bottom: 5px;
            padding-right: 30px;
            font-size: 12pt;
            font-family: Tahoma, Arial;
        }

        #en-markup-alert-container .cell-2 .cell-2-message {
            padding-right: 30px;
            font-size: 9.5pt;
            font-family: Tahoma, Arial;
        }

        #en-markup-alert-container .cell-3 {
            position: relative;
            width: 450px;
            height: 60px;
            float: left;
            background-color: rgb(240,240,240);
        }

        #en-markup-alert-container .cell-3 button {
            position: absolute;
            top: 12px;
            right: 15px;
            width: 110px;
            height: 36px;
        }

        #en-markup-alert-container .cell-3 button.alt-button {
            position: absolute;
            top: 12px;
            right: 140px;
            width: 110px;
            height: 36px;
        }
    </style></head>
<body class="min-vh-100 d-flex flex-column">
<header class="flex-shrink-0">
    <nav class="navbar navbar-expand-md navbar-dark bg-dark">
        <a class="navbar-brand" href="https://php-l3-page-analyzer.herokuapp.com">Анализатор страниц</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a class="nav-link " href="/">Главная</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="/urls">Сайты</a>
                </li>
            </ul>
        </div>
    </nav>
</header>

<main class="flex-grow-1">
    <div class="container-lg">
        <h1 class="mt-5 mb-3">Сайт: {{ $name->name }}</h1>
        <div class="table-responsive">
            <table class="table table-bordered table-hover text-nowrap">
                <tbody><tr>
                    <td>ID</td>
                    <td>{{ $name->id }}</td>
                </tr>
                <tr>
                    <td>Имя</td>
                    <td>{{ $name->name }}</td>
                </tr>
                <tr>
                    <td>Дата создания</td>
                    <td>{{ $name->created_at }}</td>
                </tr>
                </tbody></table>
        </div>
        <h2 class="mt-5 mb-3">Проверки</h2>
        <form method="post" action="https://php-l3-page-analyzer.herokuapp.com/urls/1/checks">
            <input type="hidden" name="_token" value="BU8dfaSnQnMvrRFNaEEUe1lNTzjB9Sdcibv6kv0F">
            <input type="submit" class="btn btn-primary" value="Запустить проверку">
        </form>
        <table class="table table-bordered table-hover text-nowrap">
            <tbody>
            <tr>
                <th>ID</th>
                <th>Код ответа</th>
                <th>h1</th>
                <th>keywords</th>
                <th>description</th>
                <th>Дата создания</th>
            </tr>
            <tr>
                <td>924</td>
                <td>200</td>
                <td>One worksp...</td>
                <td></td>
                <td>A new tool that blends your ev...</td>
                <td>2021-10-30 08:33:49</td>
            </tr>
            </tbody></table>
    </div>
</main>

<footer class="border-top py-3 mt-5 flex-shrink-0">
    <div class="container-lg">
        <div class="text-center">
            <a href="https://hexlet.io/pages/about" target="_blank">Hexlet</a>
        </div>
    </div>
</footer>


<iframe src="about:blank" frameborder="0" style="border: 0px; visibility: hidden; z-index: 2147483647; position: fixed; width: 100%; left: 0px; bottom: 0px; height: 300px;" id="FirebugUI"></iframe><div id="gadsf21g"><div><div class="Font-sc-zlhigq idauOE"></div></div></div></body><script src="chrome-extension://ehemiojjcpldeipjhjkepfdaohajpbdo/firebug-lite.js" id="FirebugLite" firebugignore="true" extension="Chrome" extension-id="ehemiojjcpldeipjhjkepfdaohajpbdo"></script><script src="chrome-extension://ehemiojjcpldeipjhjkepfdaohajpbdo/googleChrome.js"></script></html>
